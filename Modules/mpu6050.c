 ...
 * |
 * | Kalman filter algorithm used from
...
// Setup MPU6050
#define MPU6050_ADDR 0xD0  // 0x68 -> 0xD0 , 0x69 -> 0xD2
...
uint8_t MPU6050_Init(MPU6050_t *DataStruct, I2C_HandleTypeDef *I2Cx) {
  ...
  if (check ==
      104)  // 0x68 will be returned by the sensor if everything goes well
  {
...
void MPU6050_Start_Calibration(MPU6050_t *DataStruct) {
  DataStruct->getErrorFlag = 1;
  DataStruct->sumZeroError = 0;
  DataStruct->getErrorCount = 0;
}
...
void MPU6050_Read_All(MPU6050_t *DataStruct) {
  ...
  if (HAL_I2C_Mem_Read(I2Cx, MPU6050_ADDR, ACCEL_XOUT_H_REG, 1, Rec_Data, 14,
                       i2c_timeout))
    return;
  ...
  double pitch =
      atan2(-DataStruct->Accel_X_RAW, DataStruct->Accel_Z_RAW) * RAD_TO_DEG;
  ...
  if ((pitch < -90 && DataStruct->KalmanAngleY > 90) ||
    ...
    DataStruct->KalmanAngleY =
        Kalman_getAngle(&KalmanY, pitch, DataStruct->Gy, dt);
  }
